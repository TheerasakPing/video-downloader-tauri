...
export default function LibraryPanel() {
  ...
  if (detail) {
    return (
      <div className="p-4">
        <SeriesDetail detail={detail} onBack={closeDetail} onRemove={remove} onRefetch={handleRefetch} />
      </div>
    );
  }
    ...
    <div className="p-4 space-y-4">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {filtered.map(entry => (
            <LibraryCard key={entry.id} entry={entry} onClick={loadDetail} onRemove={remove} />
          ))}
        </div>
      )}
    </div>
