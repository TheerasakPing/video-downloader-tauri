...
import {
  ...
  ListOrdered, Loader2, CheckCircle, Image as ImageIcon, BookOpen, Compass,
} from "lucide-react";
import {
...
function App() {
  ...
  const [mergeState, setMergeState] = useState({
    isMerging: false, mergedFile: null as string | null, mergeError: null as string | null,
  ...
  const onBatchStart = useCallback((_items: BatchItem[]) => {
    // Handled below by useBatchProcessor — this callback enables the batch mode
  }, []);
  ...
  const {
    ...
    toggleBatchProcessing, resetBatchFlags,
  } = useBatchProcessor({ isFetching, setIsFetching, isDownloading: false, runDownload: async () => false, log, error });

  // --- Download manager (needs batch mode reference) ---
  const isBatchModeRef = useRef(isBatchMode);
  isBatchModeRef.current = isBatchMode;

  const {
    downloadState, setDownloadState, progress, setProgress,
  ...
  // Wire up download manager to batch processor
  useEffect(() => {
    // Update batch processor's runDownload reference when it changes
    // This is done through the isDownloading dep that batch processor uses
  }, [runDownload]);
  ...
  }, [isAutoCapture, extractUrls, log, success, setIsBatchMode, setBatchQueue, setIsBatchProcessing]);
  ...
  useEventListeners({
    ...
    onDownloadResult: useCallback(() => {}, []),
    onMergeStarted: useCallback(() => {
      setMergeState((prev) => ({ ...prev, isMerging: true, progress: 0 }));
    }, []),
    onMergeProgress: useCallback((p: { percentage: number; currentTime: number; totalDuration: number }) => {
      setMergeState((prev) => ({ ...prev, progress: p.percentage, currentTime: p.currentTime, totalDuration: p.totalDuration }));
    }, []),
    onMergeComplete: useCallback((file: string) => {
      setMergeState((prev) => ({ ...prev, isMerging: false, mergedFile: file, progress: 100 }));
      success(`Videos merged: ${file.split("/").pop()}`);
      refreshFiles();
    }, [success, refreshFiles]),
    onMergeError: useCallback((err: string) => {
    ...
    onDetectionProgress: useCallback((p: { message: string; progress: number }) => {
      setDetectionState({ isDetecting: true, message: p.message, progress: p.progress });
      if (p.progress >= 100) setTimeout(() => setDetectionState({ isDetecting: false, message: "", progress: 0 }), 500);
    }, []),
  });
      ...
      <main className="flex-1 overflow-hidden">
        {activeTab === "download" && (
          <div className="h-full flex flex-col relative">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1.5">
                  {batchQueue.length === 0 ? (
                    ...
                    batchQueue.map((item, idx) => (
                      <div key={idx} className={`p-2 rounded-lg border flex gap-2.5 text-xs relative group transition-all ${
            ...
            {isBatchMode && <div className="fixed inset-0 bg-black/30 z-40" onClick={() => setIsBatchMode(false)} />}
          </div>
        )}
        {activeTab === "library" && (
